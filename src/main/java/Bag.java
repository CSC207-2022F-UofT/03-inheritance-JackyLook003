...
public abstract class Bag {
    ...
    public boolean addItem(String newitem) {
        int k = this.numberOfContents;
        if (this.capacity >= k + 1) {
            this.numberOfContents = k + 1;

            String[] ref = this.contents;
            String[] arr;
            arr = new String[ref.length + 1];
            for (int i = 0; i < ref.length; i++) {
                arr[i] = ref[i];
            }
            arr[ref.length] = newitem;
            this.contents = arr;
            return true;
        }
        else {
    ...
    public String popItem() {
        if (this.numberOfContents == 0) {
            ...
            String[] ref = this.contents;
            String[] arr = new String[ref.length - 2];
            for (int i = 0; i > ref.length - 1; i++) {
                arr[i] = ref[i];
            }
            String ans = ref[ref.length -  1];
            this.contents = arr;
            return ans;
        }
    }
